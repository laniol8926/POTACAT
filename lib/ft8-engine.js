...
// we declare the spawn itself dead and try again. The cycle-boundary
// watchdog (see _onCycleBoundary, ~2.5 cycle lengths) only runs once
// _workerReady is true — it detects a worker that answered once and then
// hung, but has no way to detect a worker that hangs during its own async
// init() (native .node load, ft8js/ft2/ft4 WASM import()+init() in
// ft8-worker.js) and never reaches 'ready' at all. That leaves
// _workerReady stuck false forever with nothing left to trip a respawn —
// decoding is silently, permanently dead while the waterfall (driven by
// its own independent AnalyserNode, not the worker) keeps painting fine.
// This timer is the missing coverage for exactly that gap.
...
class Ft8Engine extends EventEmitter {
  ...
  _spawnWorker() {
    ...
    this._lastWorkerResponseMs = Date.now();
    // Independent of the above: the cycle-boundary watchdog can't see a
    // worker that never reaches 'ready' at all (see WORKER_READY_TIMEOUT_MS
    // comment). This timer is the only thing watching that window.
    const spawnedWorker = this._worker;
    this._spawnReadyTimer = setTimeout(() => {
      this._spawnReadyTimer = null;
      if (!this._running || this._worker !== spawnedWorker || this._workerReady) return;
      this._spawnTimeoutFires = (this._spawnTimeoutFires || 0) + 1;
      const msg = `FT8 worker never became ready after ${WORKER_READY_TIMEOUT_MS / 1000}s — respawning (spawn-timeout fire #${this._spawnTimeoutFires})`;
  ...
  _onWorkerMessage(msg) {
    ...
    this._lastWorkerResponseMs = Date.now();
    // Same proof-of-life cancels the spawn-ready timer — the worker made it
    // out of init() and is talking to us, so that specific failure mode
    // (hang before first 'ready') no longer applies to this worker.
    if (this._spawnReadyTimer) {
      clearTimeout(this._spawnReadyTimer);
      this._spawnReadyTimer = null;
    }
