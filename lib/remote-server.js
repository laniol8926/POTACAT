...
const ALLOWED_FILES = new Set([
  'remote.html', 'remote.js', 'remote.css', 'cq-target.js', 'jtcat-parser.js', 'waterfall.js',
]);
...
`;
  ...
  _buildInlinedHtml(fromTunnel = false) {
    ...
    try {
      ...
      // Shared GPU waterfall (renderer/waterfall.js, WebGL1) — same
      // component the desktop JTCAT/SSTV popouts use, shared here too
      // instead of ft8RenderWaterfall's own hand-rolled canvas colorer.
      const waterfallJs = fs.readFileSync(path.join(this._basePath, 'waterfall.js'), 'utf8');
      html = html.replace('<!-- waterfall-js -->', () => '<script>' + String.fromCharCode(10) + waterfallJs + String.fromCharCode(10) + '</script>');
    } catch (err) {
