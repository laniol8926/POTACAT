...
(function() {
  ...
  function buildMultiWaterfalls() {
    ...
    multiSliceConfigs.forEach(function(cfg) {
      ...
      // GPU waterfall (renderer/waterfall.js, WebGL1) — same shared
      // component the SSTV popout uses and the main JTCAT waterfall above
      // uses. historyRows must stay a power of two (WebGL1 REPEAT
      // constraint on the ring buffer's row axis — see waterfall.js).
    ...
    function drawMultiWf() {
      for (var p of multiWfPanes) {
        ...
        // Frequency-domain averaging (see boxAverageBins above), then let
        // the component's own adaptive floor+peak ranging handle the rest.
        ...
        p.wf.pushFrame(binsSmoothed.subarray(0, passbandBins));
      }
  ...
  // GPU waterfall (renderer/waterfall.js) — WebGL1, not the WebGL2 version
  // tried and reverted earlier tonight. Confirmed via a direct diagnostic
  // (app.getGPUInfo() + a real getContext('webgl2') probe, then a headless
  // pushFrame()+readPixels() render check) that WebGL2 is genuinely
  // unavailable on this hardware ("ES3 is blocklisted/disabled/unsupported
  // by driver") while WebGL1 renders correctly. The component owns its own
  // resize via ResizeObserver — no manual canvas.width/height bookkeeping
  // needed, unlike the canvas 2D version this replaces.
  ...
  function popoutWaterfallLoop() {
    ...
    try {
      ...
      // integrate this frame for the wfLinesPerSec pacing. At the default
      // 60 lines/sec a line is pushed every frame; slower settings
      // integrate several (bin-averaged) frames per pushed row. Averaged
      // and accumulated over ONLY the passband (0-3000Hz, passbandBins
      // wide) — pushFrame() resamples whatever it's given across the
      // component's full bin count, so passing the full 0-nyquist
      // freqData/freqDataSmoothed range (as a first cut of this did) meant
      // only the first ~half of the texture (0-3000Hz, where the real
      // signal is) ever got real content and the rest rendered black —
      // confirmed live, partial.png 2026-09-03.
      var freqDataSmoothed = boxAverageBins(freqData, passbandBins, WF_BINS_PER_OUTPUT);
      if (!wfAccum || wfAccum.length !== passbandBins) {
        wfAccum = new Float32Array(passbandBins);
        wfAccumCount = 0;
      }
      for (var ai = 0; ai < passbandBins; ai++) wfAccum[ai] += freqDataSmoothed[ai];
      wfAccumCount++;
      ...
      if (dueForLine && jpWaterfallGl.supported) {
        ...
        for (var ai2 = 0; ai2 < wfAccum.length; ai2++) wfAccum[ai2] *= invCount;
      ...
      // (bin-averaged — see boxAverageBins above) passband bins,
      // unstretched — the receiving Waterfall component (renderer/
      // remote.js) resamples to its own bin count itself, same as this
      // popout's own jpWaterfallGl.pushFrame() above.
      popoutSpectrumFrame++;
      if (popoutSpectrumFrame % 6 === 0) {
        var specBins = new Array(passbandBins);
        for (var sx = 0; sx < passbandBins; sx++) specBins[sx] = freqDataSmoothed[sx];
