...
(function () {
  ...
  // GPU waterfall (renderer/waterfall.js) — WebGL1, not WebGL2 (confirmed
  // via a direct GPU diagnostic that WebGL2 is genuinely unavailable on
  // this project's own ARM SBC target hardware; WebGL1 works). Same shared
  // component the desktop JTCAT/SSTV popouts use. `bins` is the shack's
  // raw spectrum output forwarded over the wire (see 'jtcat-spectrum'
  // above) — already frequency-bin-averaged at the source (lib/
  // spectrum-fft.js / renderer/jtcat-popout.js's boxAverageBins()), so no
  // further averaging needed here; pushFrame() handles its own adaptive
  // floor+peak ranging.
  //
  // Waterfall's default floor/peak decay constants are tuned for a ~60fps
  // push rate (the desktop popout's real rate). ECHOCAT's spectrum arrives
  // at only ~10fps (main.js's SPECTRUM_INTERVAL_MS=100 / the popout's own
  // ~10fps forward throttle), so using the same per-push constants means
  // ~6x slower real-time convergence — confirmed live (bad2.png/bad3.png,
  // 2026-09-03): the background stayed a solid mid-blue no matter how far
  // RX gain was turned down, because the floor simply hadn't caught up.
  // These are derived to match the SAME real-time convergence speed as the
  // 60fps default, not guessed: solving (1-k)^N = 1/e for the number of
  // pushes N a given time constant takes at each rate, then re-solving for
  // the per-push k that gives the same wall-clock time constant at 10fps.
  //   floor:  60fps default 0.05 -> time-constant ~0.325s -> 10fps needs ~0.265
  //   peak:   60fps default decay 0.97 -> time-constant ~0.547s -> 10fps needs decay ~0.833 (attack ~0.167)
  const FT8_WF_FLOOR_SMOOTHING_10FPS = 0.265;
  const FT8_WF_PEAK_DECAY_10FPS = 0.833;
  const FT8_WF_PEAK_ATTACK_10FPS = 0.167;
  let ft8Wf = null;
  function ft8RenderWaterfall(bins) {
    if (!bins || !bins.length) return;
    if (!ft8WfVisible) return;
    if (!ft8Wf) {
      ft8Wf = new Waterfall(ft8Waterfall, {
        bins: 256, historyRows: 256, colormap: 'classic', gamma: 1,
        floorSmoothing: FT8_WF_FLOOR_SMOOTHING_10FPS,
        peakDecay: FT8_WF_PEAK_DECAY_10FPS,
        peakAttack: FT8_WF_PEAK_ATTACK_10FPS,
      });
    }
